...
# 计算 mIoU 和 MPA
def compute_miou_mpa(preds, labels,name, num_classes=2):
    hist = np.zeros((num_classes, num_classes), dtype=np.int64)
    len1=len(preds)
    Dice = []
    miou = []
    MPA = []
    for pred, label,j in zip(preds, labels,name):
...
def evaluate(label_dir, pred_dir, threshold=128):
    ...
    miou, mpa = compute_miou_mpa(preds_all, labels_all,name)

    print(f"\n{pred_dir[-3:]}评估结果：")
    print(f"{np.mean(dice_scores):.4f}")
    print(f"{miou:.4f}")
    print(f"{mpa:.4f}")
    print(f"{np.mean(aji_scores):.4f}")
    return {
...
if __name__ == "__main__":
    # label_dir = r"D:\python_new\ggb_new_25_1021\hainan_and_segmentation_251021\ggb_new_25_1021\train\val\label" # 真实标签路径

    label_dir = r"D:\python_new\ggb_new_25_1021\hainan_and_segmentation_251021\ggb_new_25_1021\via_train\test\label" # 真实标签路径
    pred_dir = r"D:\python_new\ggb_new_25_1021\segmentation\batch_eval_xiaorong_wu\_best_9"   # 预测图路径
