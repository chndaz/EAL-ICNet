...
#通道分离
def Split(x):
    c = int(x.size()[1])# x.size()[1] = c channel
    c1 = round(c * 0.5)  #通道分为2分支
    x1 = x[:, :c1, :, :].contiguous()    #n 不变 ，c变为0-c1  ，h不变，w不变       #contiguous把tensor变成在内存中连续分布的形式
    x2 = x[:, c1:, :, :].contiguous()    #n 不变 ，c变为c1-c  ，h不变，w不变
    return x1, x2 


def Merge(x1,x2):
    return torch.cat((x1,x2),1)  #按列拼接
    
#通道打乱     ######参考 channel shuffle######
def Channel_shuffle(x,groups):
    batchsize, num_channels, height, width = x.data.size() #[样本数N, 通道数C, 高度H, 宽度W]
    
    channels_per_group = num_channels // groups  #每组通道数 。groups=2
    
    #reshape  #'''Channel shuffle: [N,C,H,W] -> [N,g,C/g,H,W] -> [N,C/g,g,H,w] -> [N,C,H,W]'''
    x = x.view(batchsize,groups,
        channels_per_group,height,width)
    
    x = torch.transpose(x,1,2).contiguous()# 交换1和2维度 [N,g,C/g,H,W] -> [N,C/g,g,H,w]
    
    #flatten
    x = x.view(batchsize,-1,height,width) #[N,C,H,W]
    
    return x


class PermutationBlock(nn.Module): #排列模块
    def __init__(self, groups):
        super(PermutationBlock, self).__init__()
        self.groups = groups

    def forward(self, input):
        n, c, h, w = input.size()  #[样本数N, 通道数C, 高度H, 宽度W]
        G = self.groups
        output = input.view(n, G, c // G, h, w).permute(0, 2, 1, 3, 4).contiguous().view(n, c, h, w)
        return output    #参考shuffleNet
...
class DownsamplerBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        diffY = x2.size()[2] - x1.size()[2]#2:H 3:W
        diffX = x2.size()[3] - x1.size()[3]#计算x1和x2的差别，0填充x1 使两者尺寸相同，然后进行concatnate

        x1 = F.pad(x1, [diffX // 2, diffX - diffX // 2,
                        diffY // 2, diffY - diffY // 2])

        output = torch.cat([x2, x1], 1)  #dim = 1 按列拼接  dim= 0 按行拼接
...
class SS_nbt_module_paper(nn.Module):
    ...
    def forward(self, x):
        ...
        if (self.dropout.p != 0): #如果dropout pro丢包率不为0
        ...
        out = Channel_shuffle(out,2)   ### channel shuffle groups=2
...
class APNModule(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.branch1 = nn.Sequential(
                nn.AdaptiveAvgPool2d(1),   #nn.AdaptiveAvgPool2d(output_size)
...
if __name__ == '__main__':
    ...
    model = LEDNet(classes=3).to(device)
    # model.eval()   # ✅ 关键！
    a = torch.rand(size=(1, 3, 480, 480)).to(device)
    print(model(a).shape)
