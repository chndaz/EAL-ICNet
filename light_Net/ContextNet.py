...
class Shallow_net(nn.Module):
    def __init__(self, in_channels,dw_channels1=32, dw_channels2=64, out_channels=128, **kwargs):
        super(Shallow_net, self).__init__()
        self.conv = Custom_Conv(in_channels, dw_channels1, 3, 2)
...
class ContextNet(nn.Module):
    def __init__(self,in_channels, classes, aux=False, **kwargs):
        super(ContextNet, self).__init__()
        self.aux = aux
        self.spatial_detail = Shallow_net(in_channels,32, 64, 128)
...
if __name__ == '__main__':
    ...
    model = ContextNet(in_channels=1,classes=19).to(device)
    summary(model,(1,512,1024))
