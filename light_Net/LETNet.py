...
from segmentation.light_Net.module.transformer import TransBlock
from segmentation.light_Net.module.patch import reverse_patches
...
class LETNet(nn.Module):
    def __init__(self,inchanel, classes=19, block_1=3, block_2=12, block_3=12, block_4=3, block_5 = 3, block_6 = 3):
        super().__init__()
        self.init_conv = nn.Sequential(
            Conv(inchanel, 32, 3, 1, padding=1, bn_acti=True),
...
if __name__ == '__main__':
    ...
    model = LETNet(classes=1).to(device)
    a = torch.rand(size=(1, 3, 512, 512)).to(device)
    print(model(a).shape)
